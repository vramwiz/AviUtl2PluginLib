...
uses ListViewEditPluginLib,ListViewEditPlugin;
...
procedure TListViewRTTI.DrawBack(cv: TCanvas; Item: TListItem; Rect: TRect;  State: TOwnerDrawState);
var
  r : TRect;
  i : Integer;
begin
  ...
  // ---- 固定列（左端）処理 ----
 // if FFixedStyle = fsVerticalFixedColumn then
  //begin
...
begin
  ...
  for j := 0 to Columns.Count-1 do begin            // 列数ループ
    ...
    if j = 0 then begin                             // 左端の表題の場合
      ...
      ListViewEditPlugins[dt.EditType].Draw(cv,r,dt);
  end;
