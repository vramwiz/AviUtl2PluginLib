unit DarkCheckListBox;

// VCLのチェック操作を保ったまま一覧配色とDPI寸法を共通化する。
interface

uses
  System.Classes, System.Types, System.UITypes,
  Vcl.CheckLst, Vcl.Controls, Vcl.StdCtrls,
  Winapi.Messages,
  DarkThemeDpiContext;

type
  TDarkCheckListBox = class(TCheckListBox)
  private
  ...
  public
    // 固定高のオーナー描画チェック一覧を共通配色で生成する。
    constructor Create(AOwner: TComponent); override;
    // 共有DPIコンテキストから切り離してチェック一覧を破棄する。
...
constructor TDarkCheckListBox.Create(AOwner: TComponent);
begin
  inherited;
...
destructor TDarkCheckListBox.Destroy;
begin
  SetDpiContext(nil);
  inherited;
end;

procedure TDarkCheckListBox.ApplyDpi;
var
...
procedure TDarkCheckListBox.ApplyTheme;
begin
...
procedure TDarkCheckListBox.ChangeScale(M, D: Integer;
  isDpiChange: Boolean);
begin
  inherited;
...
procedure TDarkCheckListBox.CMDarkThemeDpiChanged(var Message: TMessage);
begin
  ApplyDpi;
end;

procedure TDarkCheckListBox.CreateWnd;
begin
  inherited;
  SetWindowTheme(Handle, 'DarkMode_Explorer', nil);
end;

procedure TDarkCheckListBox.DrawItem(Index: Integer; Rect: TRect;
...
begin
  ...
  // チェック欄と状態描画はVCLへ委譲し、本文だけを共通配色で重ねる。
...
procedure TDarkCheckListBox.Notification(AComponent: TComponent;
...
procedure TDarkCheckListBox.SetDesignFontHeight(const Value: Integer);
begin
...
procedure TDarkCheckListBox.SetDesignItemHeight(const Value: Integer);
begin
...
procedure TDarkCheckListBox.SetDpiContext(const Value: TDarkThemeDpiContext);
begin
...
procedure TDarkCheckListBox.SetUseThemeFont(const Value: Boolean);
begin
...
initialization
  System.Classes.RegisterClass(TDarkCheckListBox);
