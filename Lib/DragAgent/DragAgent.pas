...
type TDragAgentCanStart = function (Sender: TObject; Button: TMouseButton; Shift: TShiftState; X, Y: Integer): Boolean of object;
...
type
  ...
  private
    ...
    FOnCanStart     : TDragAgentCanStart;
    FOnDragCancel   : TNotifyEvent;
    FOnDragging     : TNotifyEvent;
    FOnDropped      : TNotifyEvent;
  ...
  public
    ...
    property OnCanStart : TDragAgentCanStart read FOnCanStart write FOnCanStart;
    ...
    // 外部ターゲットへのドロップが成立したときに発生するイベント
    property OnDropped : TNotifyEvent read FOnDropped write FOnDropped;
  end;
...
begin
  ...
  try
    ...
    if (dwEffect <> DROPEFFECT_NONE) and Assigned(FOnDropped) then
      FOnDropped(Self);
  finally
    // 次回のD&Dで前回と同じIDataObjectを再利用しない。
    FDataObject := nil;
  end;
end;
...
begin
  ...
  if Assigned(FOnCanStart) and
     not FOnCanStart(Self, Button, Shift, X, Y) then exit;
...
begin
  ...
  begin
    ...
    begin
      StoredMedium := FMediumList[i];
      Medium := StoredMedium;

      // CF_HDROPなどのHGLOBALは受け取り側がReleaseStgMediumで解放する。
      // 所有中のハンドルをそのまま返すと二重解放になるため複製して渡す。
      if StoredMedium.tymed = TYMED_HGLOBAL then
      begin
        Medium.hGlobal := OleDuplicateData(StoredMedium.hGlobal,
          FFormatList[i].cfFormat, 0);
        Medium.unkForRelease := nil;
