...
// PMXファイルを介さず、呼出側が所有するモデルで姿勢を検証・正規化する。
function PreviewMmdAiPoseForModel(const Model: TPmxModel;
  const CurrentPose: string; Payload: TJSONObject): string;
...
begin
  ...
  try
    ...
    Operations.Add('begin_diagnostic_view');
    Operations.Add('end_diagnostic_view');
...
begin
  ...
  if Model = nil then
    Exit(ErrorJson('invalid_model', 'model must not be nil.'));
...
begin
  Result := PreviewMmdAiPoseForModel(GetCachedPmxModel(ModelFile),
    CurrentPose, Payload);
end;
...
begin
  ...
  try
    ...
    if SameText(Operation, 'begin_diagnostic_view') then
    begin
      PayloadValue := Root.GetValue('payload');
      if not (PayloadValue is TJSONObject) then
        Exit(ErrorJson('invalid_payload', 'payload must be an object.'));
      Exit(BeginMmdAiDiagnosticRequest(ModelFile, TJSONObject(PayloadValue)));
    end;
    if SameText(Operation, 'end_diagnostic_view') then
    begin
      PayloadValue := Root.GetValue('payload');
      if not (PayloadValue is TJSONObject) then
        Exit(ErrorJson('invalid_payload', 'payload must be an object.'));
      Exit(EndMmdAiDiagnosticRequest(TJSONObject(PayloadValue)));
    end;
