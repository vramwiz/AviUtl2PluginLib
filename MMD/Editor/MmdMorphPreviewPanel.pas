...
begin
  ...
  FList := TMmdDarkListBox.Create(Self);
  FList.Parent := Self;
  FList.SetBounds(8, 27, 229, 132);
  FList.OnClick := ListChanged;

  FTrack := THorizontalTrackBarControl.Create(Self);
  FTrack.Parent := Self;
  FTrack.Minimum := 0;
  FTrack.Maximum := 100;
  FTrack.Frequency := 10;
  FTrack.BackgroundColor := MmdEditorBackground;
  FTrack.ChannelColor := MmdEditorBorder;
  FTrack.DisabledColor := MmdEditorDisabledText;
  FTrack.FillColor := MmdEditorSelection;
  FTrack.ThumbColor := MmdEditorControl;
  FTrack.ThumbBorderColor := MmdEditorText;
  FTrack.TickColor := MmdEditorDisabledText;
